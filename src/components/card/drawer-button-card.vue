<template>
  <v-card class="mx-auto pa-0" color="grey darken-1" :tile="true">
    ...
    <v-container fluid grid-list-sm class="orange pa-1 pb-2 pt-2">
      ...
      >
        <!-- first column -->
        <v-flex d-flex xs5 child-flex class="pa-0 ma-0 pl-1">
        ...
        <!-- seondary column -->
        <v-flex d-flex xs7 child-flex class="pa-0 ma-0 pr-1">
          <numeric-buttons @click="onButtonClick"></numeric-buttons>

          <v-layout column class="pa-0 ma-0" style="width:63px">
            <!-- <v-flex column xs12 sm6> -->
            <v-btn-toggle class="toggle" v-model="toggle_exclusive" mandatory>
              <v-btn
                text
                :class="
                  `subtitle-2 white--text group-btn border-bottom ${
                    currentMode === 1 ? 'secondary' : 'grey darken-1'
                  }`
                "
                type="radio"
              ...
              <v-btn
                ...
                :class="
                  `subtitle-2 white--text group-btn border-bottom ${
                    currentMode === 2 ? 'secondary' : 'grey darken-1'
                  }`
                "
                style="border-radius: 0; height: 60px;"
              ...
              <v-btn
                ...
                :class="
                  `subtitle-2 white--text group-btn border-bottom ${
                    currentMode === 3 ? 'secondary' : 'grey darken-1'
                  }`
                "
                style="border-radius: 0; height: 60px;"
            ...
            >
              <v-icon size="large" color="white" style=" width:0.7vw;">fas fa-backspace</v-icon>
            </v-btn>
          </v-layout>
        </v-flex>
      </v-layout>
