  ...
  <v-dialog v-model="showDialog" max-width="500" persistent>
    ...
    <v-card>
      ...
      <v-card-actions>
        ...
        <v-btn v-if="uploadMode == 1" color="primary" text @click="handleSave()">{{
          $t("button-text-save")
        }}</v-btn>
...
export default class ProductImageDialog extends Vue {
  ...
  private handleSave() {
    this.onSave(this.item.ProductCode, this.imageFile);
    //this.version++;
    this.showDialog = false;
  }
  ...
  get thumbnail() {
    ...
    return this.imageId == 0
      ...
      : BASE_URL + `/api/V1/GetPosProductImage?imageId=` + this.imageId + "&ver=" + ver;
  }
