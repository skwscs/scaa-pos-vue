<template>
  <v-app-bar app flat color="indigo" dark fade-img-on-scroll>
    <v-toolbar-title>{{ appbarTitle }}</v-toolbar-title>

    <v-spacer></v-spacer>

    <div v-for="(appbarItem, i) in appbarItems" :key="i">
      <v-btn text class="my-4 appbariconsitems-hover" :to="appbarItem.link">
        <h5 class="white--text subtitle-1 font-weight-regular">{{ appbarItem.name }}</h5>
      </v-btn>
    </div>
    <v-btn text @click.native.prevent="doLogout" class="my-4">
