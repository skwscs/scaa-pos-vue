<template>
  <div>
    ...
    <v-card flat class="grey lighten-3" style="display: flex; align-items: center;">
      <v-tabs
        v-model="tabs"
        style="max-width:80%;"
        show-arrows
      ...
      >
        ...
        <v-tab v-for="n in tabLength" :key="n">
          <v-btn v-if="tabLength > 1" icon @click="onRemoveTab(n)" style="width: 36px;">
            <v-icon size="small">close</v-icon>
          </v-btn>
          {{ getCustomer(n) }}
        </v-tab>
      </v-tabs>
      <v-btn icon @click="onAddTab()" style="width: 36px;">
        <v-icon>add</v-icon>
      </v-btn>
    </v-card>
    <!-- tabPage content -->
